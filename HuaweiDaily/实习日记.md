...
* <p id="gitssl"></p>git clone提示``fatal:unable to access ': SSL certicate problem: unable to get local issuer certificate``时，由于电脑没有安装对应的ca证书，所以无法通过https连接到git的服务器，可以设置``git config --global https.sslVerify false``便可以成功clone，这里尽量不适用--global只对当前项目设置验证为false更为安全
* <p id="c/c++"></p>
