...
&emsp;&emsp;Gadelha等人[[27]](#cite-27)使用全连接层将输入图像编码为潜在表示和姿势编码，然后将姿势编码用作2D投影模块的输入，该模块将估计的3D体素渲染到输入的视图上。另一方面，Insafutdinov和Dosovitskiy [[77]](#cite-77)获取同一个对象的两个视图，从第一个视图预测相应的形状(表示为点云)，从第二个视图预测相机姿势(表示为四元数)；然后，该方法使用可微投影模块从预测的相机姿态生成预测形状的视图。形状和姿态预测器被实现为具有两个分支的卷积网络，该网络从一个卷积编码器开始，共有7层，随后是2个共享的全连接层，然后网络分成用于形状和姿势预测的两个分支，姿势分支被实现为多层感知器。
